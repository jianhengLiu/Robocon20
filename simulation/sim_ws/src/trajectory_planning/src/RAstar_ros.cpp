...
//Default Constructor
    ...
    void RAstarPlannerROS::initialize(std::string name, costmap_2d::Costmap2DROS *costmap_ros)
    {
        ...
        if (!initialized_)
        {
            ...
            MyExcelFile << "StartID\tStartX\tStartY\tGoalID\tGoalX\tGoalY\tPlannertime(ms)\tpathLength\tnumberOfCells\t"
                        << endl;
                                    ...
                                    std::vector<geometry_msgs::PoseStamped> &plan)
    {
        ...
        if (isStartAndGoalCellsValid(startCell, goalCell))
        {
            ...
            if (bestPath.size() > 0)
            {
                ...
                for (; it != plan.end(); ++it)
                {
                    // hypot(deltax,deltay):返回sqrt(deltax*deltax,deltay*deltay)
    ...
    vector<int> RAstarPlannerROS::RAstarPlanner(int startCell, int goalCell)
    {
        ...
        cout << "time to generate best global path by Relaxed A* = "
             << (diff(time1, time2).tv_sec) * 1e3 + (diff(time1, time2).tv_nsec) * 1e-6 << " microseconds" << endl;
...
/*********************************************************************************/
    vector<int> RAstarPlannerROS::findPath(int startCell, int goalCell, float g_score[])
    {
        ...
        //Open List:OPL
        //multiset 会将数据自动排序，默认匹配第一位，然后排序后一位，从低到高
        multiset<cells> OPL;
        int currentCell;
...
/*********************************************************************************/
    ...
    {
        /**
         * 这部分函数有待优化，可以在结构体中添加指向父节点和子节点的成员，
         * 这样搜索只需要逆向查询就好，不需要再次比较
         */
        ...
        while (currentCell != startCell)
        {
            ...
            //min_element:返回数列中的最小值
            //distance：返回两数列间的下标距离
...
/*********************************************************************************/
    void
    RAstarPlannerROS::addNeighborCellToOpenList(multiset<cells> &OPL, int neighborCell, int goalCell, float g_score[])
    {
        cells CP;
